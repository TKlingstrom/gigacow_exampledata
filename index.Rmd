...
Gigacow_Cow_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "Gigacow_Cow_DataView")) %>%
...
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrGigacow_Cow = count(Gigacow_Cow_Id), nrAnimals = n_distinct(SE_Number))
...
#The below code chunks run an SQL query on each table and then downloads the result.
CowMilkYieldCommon_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "CowMilkYield_Common_View")) %>%
...
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrMilkings = count(Id), nrAnimals = n_distinct(SE_Number))
...
#The below code chunks run an SQL query on each table and then downloads the result.
MilkRobot_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "MilkRobot_DataView")) %>%
...
#  summarise(count = n_distinct(Feed_Id), .groups = 'drop') #%>%
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrMilkings = count(MilkRobot_Id), nrAnimals = n_distinct(SE_Number))
...
#The below code chunks run an SQL query on each table and then downloads the result.
MilkOther_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "MilkOther_DataView")) %>%
...
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrMilkings = count(MilkOther_Id), nrAnimals = n_distinct(SE_Number))
...
#  summarise(count = n_distinct(Activity_Id), .groups = 'drop') #%>%
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrActivity = count(Activity_Id), nrAnimals = n_distinct(SE_Number))
...
#The below code chunks run an SQL query on each table and then downloads the result.
Feed_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "Feed_DataView")) %>%
...
#  summarise(count = n_distinct(Feed_Id), .groups = 'drop') #%>%
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrFeed = count(Feed_Id), nrAnimals = n_distinct(SE_Number))
...
#The below code chunks run an SQL query on each table and then downloads the result.
BCS_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "Health_BCS_DataView")) %>%
...
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrBCS = count(Health_BCS_Id), nrAnimals = n_distinct(SE_Number))
BCS = collect(BCS_Con)
...
#The below code chunks run an SQL query on each table and then downloads the result.
Health_DiagnosisTreatment_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "Health_DiagnosisTreatment_DataView")) %>%
  slice_sample(n = 100)
...
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrHealth_DiagnosisTreatment = count(Health_DiagnosisTreatment_Id), nrAnimals = n_distinct(SE_Number))
...
#The below code chunks run an SQL query on each table and then downloads the result.
Reproduction_Abortion_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "Reproduction_Abortion_DataView")) %>%
...
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrReproduction_Abortion = count(Reproduction_Abortion_Id), nrAnimals = n_distinct(SE_Number))
...
#The below code chunks run an SQL query on each table and then downloads the result.
Reproduction_Calving_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "Reproduction_Calving_DataView")) %>%
...
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrReproduction_Calving = count(Reproduction_Calving_Id), nrAnimals = n_distinct(SE_Number))
...
#The below code chunks run an SQL query on each table and then downloads the result.
Reproduction_DryOff_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "Reproduction_DryOff_DataView")) %>%
...
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrReproduction_DryOff = count(Reproduction_DryOff_Id), nrAnimals = n_distinct(SE_Number))
...
#The below code chunks run an SQL query on each table and then downloads the result.
Reproduction_Insemination_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "Reproduction_Insemination_DataView")) %>%
...
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrReproduction_Insemination = count(Reproduction_Insemination_Id), nrAnimals = n_distinct(SE_Number))
...
#The below code chunks run an SQL query on each table and then downloads the result.
Reproduction_Lactation_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "Reproduction_Lactation_DataView")) %>%
...
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrReproduction_Lactation = count(Reproduction_Lactation_Id), nrAnimals = n_distinct(SE_Number))
...
#The below code chunks run an SQL query on each table and then downloads the result.
Reproduction_PregnancyCheck_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "Reproduction_PregnancyCheck_DataView")) %>%
...
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrReproduction_PregnancyCheck = count(Reproduction_PregnancyCheck_Id), nrAnimals = n_distinct(SE_Number))
...
#The below code chunks run an SQL query on each table and then downloads the result.
Reproduction_ReproductionStatus_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "Reproduction_ReproductionStatus_DataView")) %>%
...
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrReproduction_ReproductionStatus = count(Reproduction_ReproductionStatus_Id), nrAnimals = n_distinct(SE_Number))
...
#The below code chunks run an SQL query on each table and then downloads the result.
Stillborn_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "Stillborn_DataView")) %>%
  slice_sample(n = 100)
...
#The below code chunks run an SQL query on each table and then downloads the result.
Traffic_Con <- con %>% tbl(in_catalog("Gigacow_QA", "science", "Traffic_DataView")) %>%
  slice_sample(n = 100)
...
#  summarise(count = n_distinct(SE_Number), .groups = 'drop')
  summarise(nrTraffic = count(Traffic_Id), nrAnimals = n_distinct(SE_Number))
