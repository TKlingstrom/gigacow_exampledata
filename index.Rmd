...
MilkOther_Con <- con %>% tbl(in_catalog("Gigacow", "sciDel", "Del_Milk_Other")) %>%
  filter(MilkingDate >= "2023-04-20" & !is.na(SE_Number) & !is.na(Yield)) %>%
  slice_sample(n = 100)
...
MilkOther = con %>% tbl(in_catalog("Gigacow", "sciDel", "Del_Milk_Other")) %>%
  group_by(FarmName_Pseudo) %>%
